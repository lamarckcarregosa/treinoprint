...
const personalsFixos = ["Orlando", "Cris", "Vitoria", "Lucas", "Ana", "Pedro", "Rafael", "Bruna", "Diego", "Camila"];
...
const [msgTreino, setMsgTreino] = useState<string>("");
  ...
  // =========================
  // TREINOS MODELO (BANCO)
  // =========================

  // ✅ ALTERAÇÃO 2: carregarModelo + useEffect exatamente como você pediu
  ...
  try {
    ...
    const res = await fetch(`/api/treinos?${qs.toString()}`, { cache: "no-store" });
  ...
  // ✅ sempre recarrega quando mudar semana/dia/nivel/tipo
  ...
  const addExercicio = () => {
    ...
    setModoLivre(true); // quando você começa a digitar, vira modo livre
            ...
            <section className="bg-white rounded-2xl shadow p-6 grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="md:col-span-2">
                ...
                <p className="text-[11px] text-gray-500 mt-1">
                  Troque aqui se você for mudar a semana dos treinos.
                </p>
              </div>
...
{msgTreino ? (
  <div className="md:col-span-2 text-xs mt-2">
    <span className={treinoBanco ? "text-green-700" : "text-red-600"}>{msgTreino}</span>
  </div>
) : null}
              <div className="md:col-span-2 flex items-center gap-2 pt-2">
                <input
              ...
              {/* ✅ BLOCO SUBSTITUÍDO (treinoBanco) */}
              <div className="md:col-span-2 text-xs mt-2">
                {treinoBanco ? (
                  <span className="text-green-700">
                    Treino do banco encontrado (ID: {treinoBanco.id}).
                  </span>
                ) : (
                  <span className="text-red-600">
                    Nenhum treino no banco para essa combinação (semana/dia/nível/tipo).
                  </span>
                )}
              </div>
            </section>
            ...
            {/* Exercícios */}
            <section className="bg-white rounded-2xl shadow p-6 space-y-3">
              <h2 className="font-semibold">Exercícios {modoLivre ? "(treino livre)" : "(do banco)"}</h2>
            ...
            <button
              onClick={imprimir}
              className="no-print w-full bg-green-600 hover:bg-green-700 text-white rounded-2xl p-4 font-semibold"
            >
              Imprimir treino
            </button>
          </div>

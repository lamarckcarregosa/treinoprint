...
export default function EditarTreinoPersonalizadoPage() {
  ...
  const carregar = async () => {
    try {
      ...
      if (!treinoId || Number.isNaN(treinoId)) {
        setErro("Treino inválido");
        return;
      }

      const [resTreino, resAlunos, resExercicios] = await Promise.all([
        apiFetch(`/api/treinos-personalizados/${treinoId}`, {
          cache: "no-store",
        }),
  ...
  const salvar = async () => {
    try {
      ...
      const res = await apiFetch(`/api/treinos-personalizados/${treinoId}`, {
        method: "PUT",
        headers: {
      ...
      alert("Treino personalizado atualizado com sucesso");
      await carregar();
    } finally {
      setSalvando(false);
    }
      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-2">
            Atualize os dados do treino individual do aluno.
          </p>
        </div>

        <div className="flex gap-3 flex-wrap">
          <button
            onClick={() => router.push("/treinos-personalizados")}
          ...
          <button
            onClick={excluir}
            disabled={excluindo}
            className="bg-red-600 text-white px-5 py-3 rounded-xl disabled:opacity-60"
          >
